...
const LocalNews = () => {
    ...
    const [currentId, setCurrentId] = useState(null); // null = create mode, value = edit mode
    ...
    const fetchNews = async () => {
        try {
            const res = await API.get('/localnews');
            if (res.data.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = currentId
                ? await API.put(`/localnews/${currentId}`, formData)
                : await API.post('/localnews', formData);
    ...
    const handleConfirmDelete = async () => {
        setIsDeleting(true);
        try {
            const res = await API.delete(`/localnews/${deleteId}`);
            const data = res.data;
    ...
    const openEditModal = (newsItem) => {
        setFormData({ description: newsItem.description });
                ...
                <div className="p-8">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12">
                        <div className="flex-1">
                            ...
                            <p className="text-slate-400 text-lg">Manage local updates and announcements</p>
                        </div>
                        <button
                            onClick={openAddModal}
                            className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white px-6 py-3 rounded-lg shadow-lg shadow-blue-500/20 transition-all duration-300 hover:shadow-blue-500/40 font-semibold whitespace-nowrap"
                        >
                            <PlusIcon />
                            <span>Add News</span>
                        </button>
                    ...
                    {isLoading ? (
                        <div className="flex justify-center items-center py-32">
                            <div className="relative">
                                ...
                                <div className="relative animate-spin rounded-full h-16 w-16 border-2 border-slate-700 border-t-blue-500"></div>
                            </div>
                        </div>
                    ) : newsList.length === 0 ? (
                        <div className="text-center py-24 bg-gradient-to-br from-slate-800/50 to-slate-900/50 rounded-2xl border border-slate-700/50 backdrop-blur-sm">
                            ...
                            <p className="text-slate-400 text-xl font-medium">No news available yet</p>
                            <p className="text-slate-500 text-sm mt-2">Create your first local news update to get started</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {newsList.map((news, index) => (
                                <div key={news._id} className="group relative bg-gradient-to-br from-slate-800/60 to-slate-900/60 rounded-xl border border-slate-700/50 hover:border-blue-500/50 backdrop-blur-sm transition-all duration-300 hover:shadow-xl hover:shadow-blue-500/10 overflow-hidden flex flex-col">
                                    {/* Animated background on hover */}
                                    <div className="absolute inset-0 bg-gradient-to-br from-blue-600/0 to-cyan-600/0 group-hover:from-blue-600/5 group-hover:to-cyan-600/5 transition-all duration-300 pointer-events-none"></div>
                                    ...
                                    <div className="relative p-6 flex-grow">
                                        ...
                                        <p className="text-slate-300 leading-relaxed whitespace-pre-wrap text-sm line-clamp-6">
                                            {news.description}
                                        </p>
                                    </div>
                                    ...
                                    <div className="relative bg-gradient-to-r from-slate-800/30 to-slate-900/30 px-6 py-4 flex justify-end gap-2 border-t border-slate-700/30 backdrop-blur-sm">
                                        <button
                                            ...
                                            className="p-2.5 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition-all duration-200 group/btn"
                                            title="Edit"
                                        >
                                            <EditIcon />
                                        </button>
                            ...
                            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden transform transition-all scale-100 border border-slate-700/50">
                                <div className="px-8 py-6 border-b border-slate-700/50 flex justify-between items-center bg-gradient-to-r from-slate-800/80 to-slate-900/80 backdrop-blur-sm">
                                    <h3 className="text-2xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">
                                        {currentId ? '✎ Edit Local News' : '+ Add New Local News'}
                                    </h3>
                                ...
                                <form onSubmit={handleSubmit} className="p-8">
                                    <div className="mb-6">
                                        ...
                                        <textarea
                                            rows="7"
                                            className="w-full px-4 py-3 rounded-xl bg-slate-900/50 border border-slate-700 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-none transition-all resize-none disabled:bg-slate-900/20 disabled:opacity-50 text-slate-200 placeholder-slate-500"
                                    ...
                                    <div className="flex justify-end gap-4">
                                        ...
                                        <button
                                            type="submit"
                                            disabled={isSubmitting}
                                            className="px-6 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-semibold shadow-lg shadow-blue-500/20 transition-all duration-200 disabled:opacity-70 flex items-center gap-2"
                                        >
                                            {isSubmitting ? (
                                                <>
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
                            <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 border border-slate-700/50">
                                <div className="px-8 py-8 text-center">
                                    ...
                                    <p className="text-slate-400 mb-8">Are you sure you want to delete this news item? This action cannot be undone.</p>
